...
const CATEGORY_MAP = {
  ...
  "Weather":       ["weather","rain","flood","temperature","heatwave","storm","cyclone","fog","monsoon","drought","wind","humidity","forecast","rainfall","thunderstorm","smog","snowfall"],
  "International": ["usa","america","india","china","russia","ukraine","israel","iran","saudi","uae","uk","europe","nato","united nations","trump","modi","white house","war","ceasefire","diplomacy","foreign","global","world"],
...
No hashtags. No quotes.`;
  ...
  try {
    ...
    const body = await callGemini(bodyPrompt);
...
async function uploadImage(imageUrl, title) {
  ...
  try {
    ...
    form.append("folder", "ekawaz-news");

    // Add watermark transformation if we have a watermark URL
    if(CONFIG.WATERMARK_URL) {
      form.append("eager", `w_1200,h_630,c_fill,g_center/l_fetch:${Buffer.from(CONFIG.WATERMARK_URL).toString('base64')},w_180,g_south_east,x_15,y_15,o_85`);
    } else {
      form.append("eager", "w_1200,h_630,c_fill,g_center");
    }
    ...
    // Return eager (watermarked) URL if available, else secure_url
    return d.eager?.[0]?.secure_url || d.secure_url || sourceUrl;
  } catch(e) {
