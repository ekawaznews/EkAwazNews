#!/usr/bin/env node
// ============================================================
// Ek Awaz News — Auto-Publisher v2
// - Gemini 1.5 Pro (free tier, natural writing)
// - Anti-AI word blacklist
// - Multi-page article HTML generation
// - Watermarked images via Cloudinary
...
const FEEDS = {
  politics:      'https://news.google.com/rss/search?q=Pakistan+politics&hl=en-PK&gl=PK&ceid=PK:en',
  government:    'https://news.google.com/rss/search?q=Pakistan+government+prime+minister&hl=en-PK&gl=PK&ceid=PK:en',
  sports:        'https://news.google.com/rss/search?q=Pakistan+cricket+PSL&hl=en-PK&gl=PK&ceid=PK:en',
  international: 'https://news.google.com/rss/search?q=world+news+today&hl=en&gl=US&ceid=US:en',
  entertainment: 'https://news.google.com/rss/search?q=Pakistan+entertainment+drama&hl=en-PK&gl=PK&ceid=PK:en',
  economy:       'https://news.google.com/rss/search?q=Pakistan+economy+rupee+IMF&hl=en-PK&gl=PK&ceid=PK:en',
  weather:       'https://news.google.com/rss/search?q=Pakistan+weather+rain&hl=en-PK&gl=PK&ceid=PK:en',
  education:     'https://news.google.com/rss/search?q=Pakistan+education+university&hl=en-PK&gl=PK&ceid=PK:en',
  crime:         'https://news.google.com/rss/search?q=Pakistan+crime+police+court&hl=en-PK&gl=PK&ceid=PK:en',
  social:        'https://news.google.com/rss/search?q=Pakistan+social+issues+health&hl=en-PK&gl=PK&ceid=PK:en',
  urdu:          'https://feeds.bbci.co.uk/urdu/rss.xml',
};
...
const BATCH_PLAN = {
  1:  ['politics', 'government', 'sports'],
  7:  ['international', 'entertainment', 'economy', 'education'],
  13: ['weather', 'social', 'urdu', 'urdu', 'urdu'],
};

// Manual trigger batch — runs all categories, 3 articles
const MANUAL_BATCH = ['politics', 'sports', 'international'];
...
const AI_BANNED = [
  ...
  'shed light','sheds light','a testament to','stands as a testament',
...
// ─── MAIN ─────────────────────────────────────────────────────
async function main() {
  console.log('[AutoPost] Ek Awaz Auto-Publisher v2 starting...');
  if (!GEMINI_API_KEY) { console.error('GEMINI_API_KEY missing.'); process.exit(1); }

  const utcHour  = new Date().getUTCHours();
  const weekday  = new Date().getDay();
  const isManual = process.env.GITHUB_EVENT_NAME === 'workflow_dispatch';
  let batch      = BATCH_PLAN[utcHour];

  if (!batch) {
    if (isManual) {
      console.log('[AutoPost] Manual trigger — running test batch.');
      batch = MANUAL_BATCH;
    } else {
      console.log(`No batch for UTC ${utcHour}:00. Done.`);
      return;
    }
  }

  if (utcHour === 1 && WEEKLY_SPECIALS[weekday]) {
    batch = [WEEKLY_SPECIALS[weekday], ...batch];
    console.log(`Weekly special: ${WEEKLY_SPECIALS[weekday].label}`);
  }
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    console.log(`\n[${i+1}/${batch.length}] ${specialLabel || feedKey} | ${catInfo.category}`);

    try {
      const feed = await parser.parseURL(FEEDS[feedKey]);
      const cutoff = Date.now() - 48 * 60 * 60 * 1000;
      let items = feed.items.filter(it => {
        const pub = it.pubDate ? new Date(it.pubDate).getTime() : Date.now();
        return pub > cutoff && !usedLinks.has(it.link);
      });
      if (!items.length) items = feed.items.filter(it => !usedLinks.has(it.link));
      const item = items[0];
      if (!item) { console.log('No item found. Skipping.'); continue; }
      ...
      console.log(`Source: "${rawTitle.slice(0,65)}"`);

      // Image
      let imageUrl = extractImageFromItem(item) || await extractImageFromPage(item.link || '');
      let finalImage = '';
      if (imageUrl) {
        try { finalImage = await processAndUploadImage(imageUrl); }
        catch (e) { console.log(`Image error: ${e.message}`); finalImage = imageUrl; }
      }

      // Generate article
      const data = await generateArticle(rawTitle, rawSummary, isUrdu, postType, catInfo.category, specialLabel);
      if (!data) { console.log('Gemini returned nothing. Skipping.'); continue; }
    ...
    } catch (e) { console.error(`Error [${feedKey}]:`, e.message); }
  }

  if (published.length > 0) await updateTicker(published.map(p => p.title));

  console.log(`\nDone. ${published.length} articles published.`);
}
...
async function generateArticle(headline, summary, isUrdu, type, category, specialLabel) {
  ...
  if (type === 'editorial') typeNote = 'NEWS EDITORIAL: Analytical. Pakistani public interest angle. Clear reasoned position.';
  else if (type === 'column') typeNote = 'OPINION COLUMN: Direct, thoughtful. Unique angle relevant to Pakistani society.';
  else if (specialLabel === 'Untold Story') typeNote = 'UNTOLD STORY: Underreported human-interest angle. Focus on people affected.';
  else if (specialLabel === 'Crime Report') typeNote = 'CRIME REPORT: Factual. Incident, police response, public safety angle. No graphic language.';

  const prompt = `You are a senior journalist at Ek Awaz News, a trusted Pakistani news outlet. Write like a real journalist, not an AI assistant.
...
- 450 to 580 words
- Paragraphs only. No bullet points. No numbered lists.
- Maximum one H2 subheading per article.
- BANNED WORDS — never use: ${AI_BANNED.slice(0,20).join(', ')}
- Never use em dashes (—). Use commas or short sentences instead.
- Do NOT open with: "In a significant", "In a major", "It is worth", "Recently,"
- First sentence: state the key fact immediately and directly.
- Vary paragraph lengths — some short (2 lines), some longer (4 lines). Never uniform.
- Use direct quotes with attribution ("said", "told reporters", "confirmed") where appropriate.
- Do NOT invent specific statistics, quotes, or names you are not certain about.
- Final paragraph: add context or implication, do NOT summarize what was already said.
- Body HTML: ONLY <p> and <h2> tags.

Output ONLY valid JSON. No markdown, no backticks, no text before or after:
{"title":"headline under 85 chars","excerpt":"2 sentence summary under 190 chars","body":"<p>...</p><p>...</p>","seoTitle":"SEO title 52-60 chars","seoDesc":"meta description 135-155 chars","tags":["tag1","tag2","tag3","tag4","Pakistan"]}`;
...
function cleanAIText(html) {
  ...
  // Remove double spaces left by removals
...
function generateArticlePage(post) {
  ...
  const dateStr  = new Date(post.date).toLocaleDateString('en-PK', { year:'numeric', month:'long', day:'numeric' });
  const readTime = Math.max(1, Math.ceil(post.body.replace(/<[^>]+>/g,'').split(' ').length / 200));
  const tagsHtml = (post.tags||[]).map(t=>`<a href="${SITE_URL}/?tag=${enc(t)}" class="tag">${esc(t)}</a>`).join('');
...
<script type="application/ld+json">{"@context":"https://schema.org","@type":"NewsArticle","headline":"${escJ(post.title)}","description":"${escJ(post.seoDesc||post.excerpt)}","image":"${post.image||''}","datePublished":"${post.date}","dateModified":"${post.lastEditedAt}","author":{"@type":"Person","name":"${escJ(post.author)}"},"publisher":{"@type":"Organization","name":"Ek Awaz News","logo":{"@type":"ImageObject","url":"${SITE_URL}/ek-awaz-logo.png"}},"mainEntityOfPage":{"@type":"WebPage","@id":"${pageUrl}"},"articleSection":"${escJ(post.category)}","keywords":"${escJ((post.tags||[]).join(', '))}"}</script>
...
.top-bar a{color:#aaa}.site-name-top{color:#fff;font-weight:700}
...
.logo-circle{width:52px;height:52px;background:var(--red);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.logo-circle span{color:#fff;font-family:'Noto Nastaliq Urdu',serif;font-size:12px;font-weight:700;text-align:center;line-height:1.2}
.logo-text h1{font-family:'Playfair Display',serif;font-size:20px;font-weight:900;color:var(--red);line-height:1}
.logo-text p{font-size:10px;color:#888;letter-spacing:2px;text-transform:uppercase;margin-top:2px}
.hnav{display:flex;gap:18px}.hnav a{font-size:13px;font-weight:600;color:var(--text);text-transform:uppercase;letter-spacing:.5px}
...
.nav-inner::-webkit-scrollbar{display:none}.nav-inner a{color:rgba(255,255,255,.9);padding:11px 15px;font-size:12px;font-weight:600;white-space:nowrap;text-transform:uppercase;letter-spacing:.5px;display:block}
.nav-inner a:hover{background:rgba(0,0,0,.2);text-decoration:none;color:#fff}
.ad-banner{background:var(--light);border:1px dashed var(--border);text-align:center;padding:10px;min-height:90px;display:flex;align-items:center;justify-content:center;color:#999;font-size:12px}
...
.in-ad{margin:30px 0;min-height:250px;background:var(--light);border:1px dashed var(--border);display:flex;align-items:center;justify-content:center;color:#999;font-size:12px}
...
.share-bar strong{font-size:14px;margin-right:4px}
.sbtn{display:inline-flex;align-items:center;gap:5px;padding:8px 14px;border-radius:4px;font-size:13px;font-weight:600;border:none;cursor:pointer;text-decoration:none}
...
.sidebar-ad{min-height:600px;background:var(--light);border:1px dashed var(--border);display:flex;align-items:center;justify-content:center;color:#999;font-size:12px;border-radius:4px;margin-bottom:24px}
...
<header style="border-bottom:3px solid var(--red)">
  <div class="header-inner">
    <a href="${SITE_URL}" class="logo-link">
      <div class="logo-circle"><span>ایک آواز</span></div>
...
<div class="wrap">
  <main>
    ...
    <div class="meta">
      <span>By <span class="meta-author">${esc(post.author)}</span></span>
...
<footer>
  <div>
    <a href="${SITE_URL}">Home</a><a href="${SITE_URL}/?cat=politics">Politics</a>
    <a href="${SITE_URL}/?cat=sports">Sports</a><a href="${SITE_URL}/?cat=international">World</a>
  </div>
  ...
  console.log(`[Page] news/${post.slug}.html`);
}

function makeSlug(t) {
  return t.toLowerCase().replace(/[^a-z0-9\s-]/g,'').trim().replace(/\s+/g,'-').replace(/-+/g,'-').slice(0,75)+'-'+Date.now().toString().slice(-6);
}
...
async function extractImageFromPage(url) {
  ...
  try {
    ...
    const og = h.match(/<meta[^>]+property="og:image"[^>]+content="([^"]+)"/i)||h.match(/<meta[^>]+content="([^"]+)"[^>]+property="og:image"/i);
    if (og) return og[1];
...
async function uploadToCloudinary(buf) {
  ...
  if (d.secure_url){console.log(`[Cloudinary] ${d.secure_url}`);return d.secure_url;}
...
main().catch(e=>{console.error('[AutoPost] Fatal:',e.message);process.exit(1);});
