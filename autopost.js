...
async function main() {
  ...
  if (!batch) {
    console.log(`[AutoPost] Unknown batch "${batchKey}". Defaulting to A.`);
    batch = BATCH_PLAN['A'];
  }

  console.log(`[AutoPost] Running Batch ${batchKey}: ${batch.join(', ')}`);

  if (batchKey === 'A' && WEEKLY_SPECIALS[weekday]) {
    batch = [WEEKLY_SPECIALS[weekday], ...batch];
    console.log(`[AutoPost] Weekly special: ${WEEKLY_SPECIALS[weekday].label}`);
  }
